...
double WCCd0(double *p1, double *p2, int np, int nNA) {
  int i;
  double crosscov, wght, acor1, acor2, trwdth = 0.0;
...
double wcc_autocorr(double *p1, int np) {
  int i;
  double autocov, wght, trwdth = 0.0;
...
// [[Rcpp::export]]
Rcpp::XPtr<DistanceFunctionPtr> WCCd0() {
  return (Rcpp::XPtr<DistanceFunctionPtr>(new DistanceFunctionPtr(&WCCd0)));
}
